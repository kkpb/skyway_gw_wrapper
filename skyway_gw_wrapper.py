...
class SkyWayGatewayWrapper(object):
    def __init__(self, key, domain, path):
        if not os.path.exists(path):
            ...
            self.__threads_stop_flag = {self.__gateway: False, self.__gstreamer: False}
            self.__subprocesses = {self.__gateway: None, self.__gstreamer: None}
    ...
    def __observer(self, key, setter, cmd):
        ...
        while True:
            time.sleep(1)
            if self.__threads[key] == None or self.__threads_stop_flag[key] or self.__subprocesses[key] == None:
